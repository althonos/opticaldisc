...
where
    H: ::std::io::Seek + ::std::io::Read,
{
    pub fn new(handle: &'a mut RefCell<H>, start: u32, length: u32) -> Self {
        handle.borrow_mut().seek(::std::io::SeekFrom::Start(start as u64));
        Self {
            handle,
            start,
            length,
    ...
    H: ::std::io::Seek + ::std::io::Read,
{
    fn read(&mut self, buffer: &mut [u8]) -> ::std::io::Result<usize> {
        ...
        let res = self.handle.borrow_mut().read(&mut buffer[..size as usize]);
